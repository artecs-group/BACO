...
dynamic_bitset::dynamic_bitset(const dynamic_bitset& db) 
    : _blockSize(db._blockSize), _mask(db._mask), _currentSize(db._currentSize)
{}


dynamic_bitset& dynamic_bitset::operator= (const dynamic_bitset& db)
{
  if(db == *this) return *this;

  _currentSize=db._currentSize;
  _mask = db._mask;
  _blockSize = db._blockSize;
